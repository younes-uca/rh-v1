...
export class DemandeCongeEditAdminComponent extends AbstractEditController<DemandeCongeDto, DemandeCongeCriteria, DemandeCongeService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.startTime = this.demandeCongeService.format(this.item.startTime);
        this.item.endTime = this.demandeCongeService.format(this.item.endTime);
    }
   ...
   public async openCreateTypeDemandeConge(typeDemandeConge: string) {
        const isPermistted = await this.roleService.isPermitted('TypeDemandeConge', 'edit');
        if(isPermistted) {
             this.typeDemandeConge = new TypeDemandeCongeDto();
   ...
   public async openCreateEtatDemandeConge(etatDemandeConge: string) {
        const isPermistted = await this.roleService.isPermitted('EtatDemandeConge', 'edit');
        if(isPermistted) {
             this.etatDemandeConge = new EtatDemandeCongeDto();
