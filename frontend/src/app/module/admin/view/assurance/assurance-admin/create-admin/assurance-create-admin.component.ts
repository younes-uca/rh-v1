...
@Component({
  selector: 'app-assurance-create-admin',
  templateUrl: './assurance-create-admin.component.html'
})
export class AssuranceCreateAdminComponent extends AbstractCreateController<AssuranceDto, AssuranceCriteria, AssuranceService>  implements OnInit {
    ...
    const isPermistted = await this.roleService.isPermitted('LienParente', 'add');
    if(isPermistted) {
         this.lienParente = new LienParenteDto();
    ...
    const isPermistted = await this.roleService.isPermitted('CompagnieAssurance', 'add');
    if(isPermistted) {
         this.compagnieAssurance = new CompagnieAssuranceDto();
