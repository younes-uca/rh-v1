...
export class StationEditAdminComponent extends AbstractEditController<StationDto, StationCriteria, StationService>   implements OnInit {
   ...
   public addDepartements() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.departementsElement.id == null){
                this.item.departements.push(this.departementsElement);
            }else{
                const index = this.item.departements.findIndex(e => e.id == this.departementsElement.id);
                this.item.departements[index] = this.departementsElement;
            }
